/**
 * Run the Python edge-align pipeline for one or all paintings.
 * Skips paintings that already have edge-corrected-placements.json (don't redo work).
 * Usage: npx tsx scripts/edge-align.ts [--id <painting-id>]
 */
...
const PYTHON_SCRIPT = path.join(process.cwd(), "scripts", "edge_align_sections", "run.py");
...
function hasPlacements(paintingId: string): Promise<boolean> {
  const placementsPath = path.join(SOURCE_IMAGES, paintingId, "edge-corrected-placements.json");
...
async function main(): Promise<void> {
  ...
  for (const id of paintingIds) {
    ...
    if (has) {
      console.log(`Skipping ${id} (already has placements).`);
      continue;
    }
    process.stdout.write(`Edge-aligning ${id}... `);
    try {
